...
local function CreateEPGPFrameStandings()
  ...
  for i,r in ipairs(rowFrame.rows) do
    ...
    r:SetScript(
      ...
      function(self)
        ...
        GameTooltip:AddDoubleLine("Звание ", GS:GetRank(self.name), 1, 1, 1, 1, 1, 1)
        -- GameTooltip:AddLine(GS:GetRank(self.name))
        if EPGP:GetNumAlts(self.name) > 0 then
          GameTooltip:AddLine(" ")
          GameTooltip:AddLine(L["Alts"])
          for w=1,EPGP:GetNumAlts(self.name) do
            GameTooltip:AddDoubleLine(" ", EPGP:GetAlt(self.name, w), 1, 1, 1, 1, 1, 1)
          end
        end
        -- GameTooltip:ClearAllPoints()
        -- GameTooltip:SetPoint("TOPLEFT", self, "TOPRIGHT")
  ...
  local function UpdateStandings()
    ...
    FauxScrollFrame_Update(EPGPScrollFrame, numMembers, numDisplayedMembers, rowFrame.rowHeight, nil, nil, nil, nil, nil, nil, true)
  ...
  EPGP.RegisterCallback(rowFrame, "StandingsChanged", function() rowFrame.needUpdate = true end)
